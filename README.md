...
It's built for Windows and runs the actual Windows machine directly: no WSL2, no gateway service, no cloud account to wire up. Point it at a local model and go. Other local agents exist and several are excellent; Hearth's focus is the zero-setup, runs-your-real-PC end of that space.
